...
export async function POST(request: NextRequest) {
  try {
    ...
    const rl = rateLimit(`generate-cas-clinique:${user.id}`, RATE_LIMITS.ai);
    if (!rl.allowed) {
    ...
    return await generateCourseSection(slug, "cas_clinique");
  } catch (error) {
    console.error("[generate/cas-clinique] Erreur non gérée:", error);
