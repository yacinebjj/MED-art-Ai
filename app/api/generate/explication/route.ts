...
export async function POST(request: NextRequest) {
  try {
    ...
    const rl = rateLimit(`generate-explication:${user.id}`, RATE_LIMITS.ai);
    if (!rl.allowed) {
    ...
    return await generateCourseSection(slug, "explication");
  } catch (error) {
    console.error("[generate/explication] Erreur non gérée:", error);
