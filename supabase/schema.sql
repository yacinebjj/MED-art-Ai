...
-- ---------------------------------------------------------------------------
-- studio_podcast_cache: cross-student cache for the Studio "Podcast Audio"
-- tab (a single ~10-15 min narrated episode, openai/gpt-audio-mini via
-- OpenRouter — see app/api/studio/podcast/route.ts). Same shape/philosophy
-- as studio_infographic_cache above: keyed by
-- sha256(normalizeText(explication)) (the SAME hash function, a different
-- table — no collision risk), one row per distinct course explication, ever,
-- across the whole shared catalog. Confirmed live (2026-09-02, a real
-- minimal calibration test): ~20 audio tokens/second, billed at
-- gpt-audio-mini's completion-token rate ($0.0000024/token) — a real episode
-- costs roughly $0.02-0.04, paid ONCE per distinct course, never per student.
-- `audio_url` is a Supabase Storage public URL (bucket "studio-podcasts",
-- created on first use) pointing at the final .mp3 — never a base64 blob.
-- ---------------------------------------------------------------------------
create table if not exists studio_podcast_cache (
  content_hash text primary key,
...
alter table studio_podcast_cache enable row level security;
drop policy if exists "Deny all client access" on studio_podcast_cache;
create policy "Deny all client access" on studio_podcast_cache for all using (false);

create or replace function increment_studio_podcast_cache_hit_count(p_content_hash text)
returns void
language sql
as $$
  update studio_podcast_cache set hit_count = hit_count + 1, last_hit_at = now() where content_hash = p_content_hash;
$$;

-- ---------------------------------------------------------------------------
-- lecture_notes_jobs: Dashboard "Audio to Smart Notes" tool — a student
-- uploads a raw lecture recording (1-2h, français/darija algérienne). A real
-- recording routinely exceeds OpenRouter's confirmed hard 50 MiB
-- request-body ceiling (see lib/ai/openrouter.ts's own comment), so the
-- CLIENT decodes and splits it into small chunks (lib/audio/browser-
-- chunking.ts), transcribes each one (app/api/lecture-notes/transcribe-chunk
-- — OpenRouter's transcription endpoint, 100% OpenRouter, no third-party STT
-- provider), then sends the joined full transcript here
-- (app/api/lecture-notes/process) for Phase 2 extraction (CHEAP_MODEL via
-- OpenRouter, see lib/ai/lecture-notes-prompts.ts).
-- NOT a cross-student cache like studio_*_cache above — every recording is
-- unique to the student who made it, so this is per-user content, same
-- philosophy/RLS shape as studio_courses. `status` lets a failed run
-- (STT/extraction error) leave a real, inspectable row instead of silently
-- disappearing. `audio_urls` is an ORDERED jsonb array of Supabase Storage
-- URLs — one per chunk (bucket "lecture-recordings", created on first use) —
-- never a single URL, since the recording only ever exists in Storage as
-- multiple small chunk files, not one big original blob.
-- ---------------------------------------------------------------------------
create table if not exists lecture_notes_jobs (
  id bigint generated by default as identity primary key,
  user_id uuid not null references auth.users (id) on delete cascade,
  title text not null,
...
create index if not exists lecture_notes_jobs_user_id_created_at_idx on lecture_notes_jobs (user_id, created_at desc);

alter table lecture_notes_jobs enable row level security;

drop policy if exists "Users manage their own lecture notes" on lecture_notes_jobs;
create policy "Users manage their own lecture notes" on lecture_notes_jobs
  for all
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);
...
create table if not exists course_workspace_cache (
  ...
  generation_type text not null check (generation_type in ('summary_chunk', 'keyword_row_v3', 'medical_dictionary_v2')),
...
--   'medical_dictionary_v1' -> a self-contained Markdown string per course
--                       (title + a 2-column Terme/Explication table, with an
--                       occasional inline darja/arabe parenthetical), same
--                       shape family as 'summary_chunk'.
--   'medical_dictionary_v2' -> same shape family, but the table is now 3
--                       columns (Terme | Explication clinique | الشرح
--                       بالعربية) — a dedicated, ALWAYS-present Arabic
--                       explanation column per term, never an occasional
--                       parenthetical — see buildMedicalDictionaryPrompt in
...
alter table course_workspace_cache add constraint course_workspace_cache_generation_type_check
  check (generation_type in ('summary_chunk', 'keyword_row_v3', 'medical_dictionary_v2'));
...
revoke execute on function increment_studio_podcast_cache_hit_count(text) from anon, authenticated;
