...
export function Sidebar({
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-40 border-r border-border bg-slate-50 transition-transform duration-300 dark:bg-slate-900",
          "lg:static lg:shrink-0 lg:translate-x-0 lg:overflow-hidden lg:border-r lg:transition-[width] lg:duration-300",
          open ? "translate-x-0" : "-translate-x-full",
          isDesktopSidebarOpen ? "lg:w-64" : "lg:w-0 lg:border-r-0"
        )}
      >
        {/* Fixed-width inner shell — the OUTER <aside> is what animates
            width (64 -> 0) on desktop collapse; if this inner content had no
            width of its own, it would reflow/squish as the outer shrinks
            instead of appearing to slide out of view. The outer's
            overflow-hidden clips it cleanly either way. */}
