...
import { InteractiveQuiz, type Qcm as QCM, type Qroc as QROC } from "@/components/course/workspace/InteractiveQuiz";

/**
 * courseSlug used for SRS attempts (app/api/srs/attempt) — this hardcoded
 * course has no real `courses` table row (its slug "appendicite" 404s
 * against /api/courses/slug), so this is just a stable, non-colliding key
 * to group its attempts under, matching the legacy slug already reserved
 * for it in lib/course-slug-content.ts.
 */
const COURSE_SLUG = "appendicite";

/* ----------------------------------------------------------------------- */
/* Phase 1 — Strict data structure (types re-exported from the shared quiz  */
/* engine, InteractiveQuiz.tsx, under their original local names below).    */
/* ----------------------------------------------------------------------- */
